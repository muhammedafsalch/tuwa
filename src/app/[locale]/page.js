    ...
    <div className="flex flex-col gap-24 lg:gap-32 pb-24">
            ...
            <div className="max-w-2xl">
              <Reveal>
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6 border-primary-500/30">
                  ...
                  <span className="text-sm font-medium text-primary-400">{dict?.hero?.badge || "Trusted. Professional. Reliable."}</span>
                </div>
              </Reveal>
              ...
              <Reveal delay={0.1}>
                <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold leading-tight mb-4 lg:mb-6 text-white">
                  ...
                  /><br/>
                  {dict?.hero?.title_2 || "in Qatar"}
                </h1>
                <p className="text-base sm:text-lg text-gray-300 mb-6 lg:mb-10 leading-relaxed max-w-xl">
                  {dict?.hero?.desc || "Helping entrepreneurs, startups, SMEs and enterprises establish and grow their business in Qatar through professional, transparent and reliable business services."}
                </p>
              </Reveal>

              <Reveal delay={0.3}>
                <div className="flex flex-col sm:flex-row gap-4 lg:gap-6">
                  <Button href={`/${locale}/request-quote`} variant="primary" className="w-full sm:w-auto text-sm sm:text-base px-6 py-3 lg:px-8 lg:py-4">{dict?.hero?.btn_primary || "Start Your Business Today"}</Button>
                  <Button href={`/${locale}/contact`} variant="outline" className="w-full sm:w-auto text-sm sm:text-base px-6 py-3 lg:px-8 lg:py-4">{dict?.hero?.btn_secondary || "Free Consultation"}</Button>
                </div>
              </Reveal>
            </div>
      ...
      <section className="container mx-auto px-6 relative z-10">
        <GlassCard className="!p-0 overflow-hidden border-white/10">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-white/10">
            {[
              { number: "10+", label: dict?.stats?.years || "Years Experience" },
              { number: "500+", label: dict?.stats?.businesses || "Businesses Served" },
              { number: "98%", label: dict?.stats?.satisfaction || "Client Satisfaction" },
              { number: "24/7", label: dict?.stats?.support || "Reliable Support" }
            ].map((stat, i) => (
      ...
      <section id="services" className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <Reveal>
            <h2 className="text-3xl lg:text-5xl font-bold mb-6">{dict?.services?.title || "Our Services"}</h2>
            <p className="text-gray-400">{dict?.services?.desc || "Comprehensive corporate solutions designed to simplify your business operations and ensure full compliance in Qatar."}</p>
          </Reveal>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { title: dict?.services?.s1?.title || "Business Setup", icon: Building2, desc: dict?.services?.s1?.desc || "End-to-end company formation and registration services.", slug: 'business-setup' },
            { title: dict?.services?.s2?.title || "PRO Services", icon: Users, desc: dict?.services?.s2?.desc || "Government liaison and documentation processing.", slug: 'pro-services' },
            { title: dict?.services?.s3?.title || "Legal Translation", icon: FileText, desc: dict?.services?.s3?.desc || "Certified legal translation for all official documents.", slug: 'legal-translation' },
            { title: dict?.services?.s4?.title || "Document Attestation", icon: ShieldCheck, desc: dict?.services?.s4?.desc || "Fast and reliable attestation from relevant ministries.", slug: 'document-attestation' },
            { title: dict?.services?.s5?.title || "Debt Recovery", icon: TrendingUp, desc: dict?.services?.s5?.desc || "Professional and compliant debt clearance services.", slug: 'debt-recovery' },
            { title: dict?.services?.s6?.title || "Immigration Services", icon: Briefcase, desc: dict?.services?.s6?.desc || "Visa processing, renewals, and labour services.", slug: 'immigration-services' },
          ].map((service, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <GlassCard className="group h-full flex flex-col">
                ...
                <Link href={`/${locale}/services/${service.slug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary-400 hover:text-white transition-colors mt-auto">
                  {dict?.services?.learn_more || "Learn More"} <ArrowRight size={16} />
                </Link>
              </GlassCard>
            </Reveal>
          ))}
        ...
        <div className="mt-12 text-center">
           <Button href={`/${locale}/services`} variant="outline">{dict?.services?.btn || "View All Services"}</Button>
        </div>
      </section>
      ...
      <section className="relative">
        ...
        <div className="container mx-auto px-6 py-24 relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <Reveal>
                <h2 className="text-3xl lg:text-5xl font-bold mb-6">{dict?.why?.title || "Why Choose Tuwa"}</h2>
                <p className="text-gray-400 mb-8 text-lg">{dict?.why?.desc || "We combine deep local expertise with international standards of professionalism to deliver unmatched corporate services."}</p>
              </Reveal>
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  dict?.why?.f1 || "Experienced Professionals", dict?.why?.f2 || "Transparent Pricing", 
                  dict?.why?.f3 || "Fast Processing", dict?.why?.f4 || "Reliable Support", 
                  dict?.why?.f5 || "Government Expertise", dict?.why?.f6 || "End-to-End Solutions"
                ].map((feature, i) => (
                  <Reveal key={i} delay={i * 0.1}>
                    <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-2 gap-8">
               {[
                 { title: dict?.why?.t1 || "Trust", icon: ShieldCheck, delay: 0.2 },
                 { title: dict?.why?.t2 || "Integrity", icon: Award, delay: 0.3 },
                 { title: dict?.why?.t3 || "Reliable", icon: CheckCircle2, delay: 0.4 },
                 { title: dict?.why?.t4 || "Professional", icon: Briefcase, delay: 0.5 }
               ].map((item, i) => (
          ...
          <div className="text-left max-w-xl z-10">
            <Reveal>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-4 border-primary-500/30">
                <span className="text-sm font-medium text-primary-400">{dict?.core?.badge || "Core Advantage"}</span>
              </div>
              <h2 className="text-3xl lg:text-5xl font-bold text-white mb-4">{dict?.core?.title || "The TUWA Difference"}</h2>
              <p className="text-gray-300 text-lg mb-8">{dict?.core?.desc || "Full Spectrum Compliance. Accelerated Delivery. Zero Hidden Costs."}</p>
              <Button href={`/${locale}/core-advantage`} variant="primary" className="px-8 py-4 text-lg">{dict?.core?.btn || "Discover Our Advantages"}</Button>
            </Reveal>
          </div>
          ...
          <div className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-4 z-10">
            <Reveal delay={0.2}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-primary-500/30 transition-colors group h-full">
                 ...
                 <h4 className="text-white font-bold text-lg">{dict?.core?.i1?.title || "100% Secure"}</h4>
                 <p className="text-gray-400 text-sm">{dict?.core?.i1?.desc || "Legally compliant"}</p>
              </div>
            </Reveal>
            <Reveal delay={0.3}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-primary-500/30 transition-colors group h-full">
                 <Clock size={32} className="text-primary-400 mb-3 group-hover:scale-110 transition-transform" />
                 <h4 className="text-white font-bold text-lg">{dict?.core?.i2?.title || "Fast Track"}</h4>
                 <p className="text-gray-400 text-sm">{dict?.core?.i2?.desc || "Accelerated setup"}</p>
              </div>
            </Reveal>
            <Reveal delay={0.4}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-primary-500/30 transition-colors group h-full">
                 <TrendingUp size={32} className="text-primary-400 mb-3 group-hover:scale-110 transition-transform" />
                 <h4 className="text-white font-bold text-lg">{dict?.core?.i3?.title || "Growth"}</h4>
                 <p className="text-gray-400 text-sm">{dict?.core?.i3?.desc || "Business expansion"}</p>
              </div>
            </Reveal>
            <Reveal delay={0.5}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-primary-500/30 transition-colors group h-full">
                 <Users size={32} className="text-primary-400 mb-3 group-hover:scale-110 transition-transform" />
                 <h4 className="text-white font-bold text-lg">{dict?.core?.i4?.title || "Expert PRO"}</h4>
                 <p className="text-gray-400 text-sm">{dict?.core?.i4?.desc || "Dedicated team"}</p>
              </div>
            </Reveal>
          </div>
        </div>
      ...
      <section className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <Reveal>
            <h4 className="text-primary-400 font-bold tracking-widest text-sm uppercase mb-4">{dict?.feedback?.badge || "Client Feedback"}</h4>
            <h2 className="text-4xl lg:text-5xl font-bold text-white">{dict?.feedback?.title_1 || "CORPORATE"} <span className="text-gradient-premium">{dict?.feedback?.title_2 || "TRUST"}</span></h2>
          </Reveal>
        </div>
      ...
      <section className="container mx-auto px-6 relative z-10">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <Reveal>
              <h4 className="text-primary-400 font-bold tracking-widest text-sm uppercase mb-4">{dict?.faq?.badge || "Support Hub"}</h4>
              <h2 className="text-3xl lg:text-5xl font-bold text-white">{dict?.faq?.title_1 || "Expert"} <span className="text-gradient-premium">{dict?.faq?.title_2 || "FAQ"}</span></h2>
            </Reveal>
          </div>
          <Reveal delay={0.2}>
            <FAQAccordion items={dict?.faq?.items || faqItems} />
          </Reveal>
        </div>
      </section>

      {/* Posters Showcase Section */}
      <Posters dict={dictObj.posters} />
            ...
            <div className="text-left max-w-2xl">
              <Reveal>
                ...
                <p className="text-gray-300 text-base sm:text-lg lg:text-xl mb-8 lg:mb-10 leading-relaxed max-w-xl">
                  {dict?.cta?.desc || "Connect with our corporate specialists today and take the first step towards a compliant and successful business launch in Doha."}
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-start w-full sm:w-auto">
                  ...
                  >
                    ...
                    {dict?.cta?.whatsapp || "WHATSAPP SUPPORT"}
