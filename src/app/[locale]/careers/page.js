...
export async function generateMetadata({ params }) {
  ...
  const dict = dictObj['careers']?.metadata;
  
  const title = dict?.title || "Careers | Tuwa Business Solutions";
  const description = dict?.description || "Join our team of experts in Qatar. We are looking for talented professionals to grow with us.";
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://tuwabusiness.com/${locale}/careers`
    }
  };
}
        ...
        <div className="max-w-3xl text-center mx-auto">
          <Reveal>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6 border-primary-500/30">
              <span className="text-sm font-medium text-primary-400">{dict?.hero?.badge || "Join Our Team"}</span>
            </div>
            <h1 className="text-4xl lg:text-5xl font-bold leading-tight mb-6">
              {dict?.hero?.title_1 || "Build Your Career with"} <br />
              <span className="text-gradient-premium">{dict?.hero?.title_2 || "Tuwa Business Solutions"}</span>
            </h1>
            <p className="text-lg text-gray-300 mb-8">
              {dict?.hero?.desc || "We are always on the lookout for talented, driven, and passionate individuals to join our growing team of corporate experts in Qatar."}
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="container mx-auto px-6 relative z-10">
        <Reveal delay={0.1}>
          <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
            <Briefcase className="text-primary-400" /> {dict?.openings?.title || "Current Openings"}
          </h2>
        </Reveal>
            ...
            <Reveal key={i} delay={i * 0.1}>
              <GlassCard className="h-full flex flex-col hover:border-primary-500/30 transition-all cursor-pointer group">
                ...
                <div className="mt-auto flex justify-between items-center border-t border-white/5 pt-4">
                  <span className="text-sm text-gray-400">{dict?.openings?.apply || "Apply below"}</span>
      ...
      <section className="container mx-auto px-6 relative z-10">
        <Reveal delay={0.2}>
          <GlassCard className="max-w-3xl mx-auto p-8 lg:p-12">
            <h3 className="text-2xl font-bold text-white mb-6">{dict?.form?.title || "Submit Your Application"}</h3>
            <p className="text-gray-400 mb-8 text-sm">{dict?.form?.desc || "Don't see a role that fits? Submit your CV anyway, and we'll keep it on file for future opportunities."}</p>
            
            <form className="flex flex-col gap-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-gray-300">{dict?.form?.name || "Full Name *"}</label>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-gray-300">{dict?.form?.email || "Email Address *"}</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300">{dict?.form?.position || "Position Applying For"}</label>
                <select className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary-500 appearance-none">
                  <option value="" className="bg-[#070B17]">{dict?.form?.general || "General Application"}</option>
                  {(dict?.jobs || []).map(job => (
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300">{dict?.form?.linkedin || "LinkedIn Profile URL"}</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300">{dict?.form?.cv || "Upload CV / Resume *"}</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300">{dict?.form?.cover || "Cover Letter"}</label>
              ...
              <Button variant="primary" type="button" className="mt-4 w-full md:w-auto self-start">
                {dict?.form?.btn || "Submit Application"}
              </Button>
            </form>
          </GlassCard>
        </Reveal>
